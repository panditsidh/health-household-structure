...
Both decision making and health care use among pregnant women increased meaningfully over time, in both household types.  Lower decision making power does not necessarily translate into lower maternal healthcare use. Autonomy and healthcare use operate through distinct mechanisms, with household resources playing a key role in enabling healthcare use.
\par
% If the contribution splits onto the second page, please add a \pagebreak here.
\noindent\textbf{CONTRIBUTION}\\
This paper documents a reversal in the trend towards nuclear family residence among pregnant women and shows that extended family residence does not necessarily impede maternal healthcare.
...
\subsubsection{Independent variable: Extended family residence}
...
\subsubsection{Dependent variables: Decision-making and health care use} \label{sec:outcomes}
...
\subsubsection{OLS linear probability models of autonomy and health care use outcomes on family structure}

We estimate OLS linear probability models to examine the association between household structure and the four binary outcomes: (1) reporting ``no say'' in decisions about the respondent's own healthcare, (2) reporting ``no say'' in decisions about visits to family or relatives, (3) having an institutional delivery (birth in a health facility), and (4) receiving at least four prenatal visits. We use these models because the coefficients can be directly interpreted as percentage point differences, which facilitates comparison across outcomes and survey rounds. In our data, the outcomes are not extremely rare, so linear models provide a reasonable approximation.
...
\small\scriptsize{\textsf{The sample is restricted to ever-married women in either nuclear or patrilocal extended household structures, and women who report being three or more months pregnant when surveyed.}}
...
Appendix \ref{appendix:hhstructure} shows that similar patterns emerge across various different samples besides pregnant women and that the share of people in natal households, the other major household structure which we omit, remains stable in the time period. 
\end{table}

\subsection{Extended family residences predicts both maternal healthcare use and autonomy}
...
\vspace{1ex}
\small\scriptsize{\textsf{\textit{Note}: Data are from NFHS rounds 3, 4, and 5.}} \small\scriptsize{\textsf{Stars indicate the statistical significance of the difference between nuclear and patrilocal extended households. P-values are from weighted linear probability models regressing each outcome on an indicator for patrilocal extended household residence.}} 
...
\subsection{Household wealth mediates the maternal healthcare use - family structure relationship, but not the autonomy - family structure relationship}
...
\sffamily
\captionsetup{style=CAPTAB}
\caption{The gap between patrilocal extended and nuclear households in women's autonomy and health care use is narrowing across survey rounds}
...
\begin{center}
\includegraphics[scale=0.7]{figures/figureC1_hhstruc_composition_allwomen.pdf}
\end{center}

\vspace{1ex}
\small\scriptsize{\textsf{\textit{Note}: Data is from the NFHS rounds 3, 4, and 5. Panel A uses the sample of ever-married women who report being 3 or more months pregnant at the time of the survey. Panel B uses the sample of ever-married women who do not report being pregnant at the time of the survey. Panel C has all women 15-49}}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{figures/figureC2_hhstruc_composition_husbands.pdf}
\end{center}

\vspace{1ex}
\small\scriptsize{\textsf{\textit{Note}: Data is from NFHS rounds 3, 4, and 5. The sample is restricted to currently married men whose wife or partner was pregnant at the time of the survey.}}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{figures/figureC3_hhstruc_composition_allendorf2013sample.pdf}
\end{center}
\label{fig:allendorf}
\vspace{1ex}
\small\scriptsize{\textsf{\textit{Note}: Data is from NFHS rounds 3, 4, and 5. The sample is restricted to married women between the ages of 15-29 who are usual residents of either a nuclear or patrilocal household, and are currently living with their husband. \cite{allendorf2013going} found rising nuclearization in NFHS 1, 2, and 3 data using the same sample restrictions in addition to restricting for only women in nuclear or patrilocal households.}}
\end{flushleft}
\end{figure}
...
\refstepcounter{section}
\subsection*{D. Kitagawa decomposition of changes in autonomy and health care indicators} \label{sec:decomposition_outcomes}
